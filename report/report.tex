...
\author{Jacob Gallop (Student ID: 20253130)}
\date{}
...
\[
    z_{n+1} = z_n^2 + \kappa, \qquad z_0 = \kappa,
\]
remains bounded. Since the set is contained within $|\kappa| < 2$, it
suffices to iterate until $|z_n| > 2$ to classify a point as outside the
set, with an upper bound of $\texttt{MAXITER} = 1000$ iterations. Increasing 
\texttt{MAXITER} computes a more accurate set of points, but increases 
the workload.
...
only the top half plus the middle row is computed for a total of
$H = (N/2+1) \times N = 112{,}515{,}000$ pixels; the lower half is
obtained by mirroring. The iteration counts are rendered 
...
After profiling the initial serial program (\texttt{serial\_0}) with 
\texttt{gprof}, Table~\ref{tab:serial_gprof} revealed that 
$87.8\%$ of execution time is spent on line 29: 
\begin{lstlisting}[firstnumber=29]
while ((cabsf(z) <= 2.0f) && (k++ < MAXITER)) { 
\end{lstlisting}
where complex magnitude \texttt{cabsf(z)} is computed as 
$|z| = \sqrt{\operatorname{Re}(z)^2 + \operatorname{Im}(z)^2}$. 
To avoid this expensive \texttt{sqrt()} operation entirely, the comparison 
is replaced with ($\operatorname{Re}(z)^2 + \operatorname{Im}(z)^2) > 4$, 
where points are computed in parametric form. This led to the following 
implementation, denoted as \texttt{serial\_1}:
...
\end{lstlisting}
which was later combined with the symmetry optimisation as \texttt{serial\_2}.
...
\bottomrule
\end{tabular}
\caption{gprof line-level profiling results ($N = 4000$)}
\label{tab:serial_gprof}
\end{table}

Table~\ref{tab:serial_perf} presents the hardware-level metrics collected 
via \texttt{perf stat}.
...
\bottomrule
\end{tabular}
\caption{perf stat hardware performance metrics ($N = 4000$)}
\label{tab:serial_perf}
\end{table}

The parametric implementation achieves a $6.9\times$ speedup, attributed to 
the reduced instruction count and increased CPU pipelining. Use of 
explicit arithmetic (vs \texttt{cabsf()}) allows the compiler to schedule 
simple floating-point math operations more efficiently, achieving an IPC 
increase of $\approx 2\times$.
The addition of the symmetry optimisation in \texttt{serial\_2} saw an 
expected speedup of $\approx 2\times$. \\

For comparison with MPI implementations, the final serial implementation 
(\texttt{serial\_2}) completed with a wall-clock time of \textbf{35.632\,s} 
using $N = 15{,}000$. All MPI benchmarks are conducted using $N = 15{,}000$ 
to remain consistent with the largest chunksizes of task 2.
...
skewed: pixels within the set boundary can require up to 1000 iterations,
whereas pixels far from the set diverge in a handful. Block partitioning
...
workers in chunks of a tuneable \texttt{chunksize}:
...
This shuffles the workload across all rows of the grid so that each rank
processes a mix of cheap and expensive pixels, naturally balancing the load.

The implementation constructs a shuffled index array on the root:
\begin{lstlisting}
...
$\{r C,\, r C{+}1,\, \ldots\}$ in cyclic order, which correspond
to every $p$-th pixel in the original domain.

After computation and \texttt{MPI\_Gather}, the root reverses pixel iterations 
into their original order, and computes pixel colours:
\begin{lstlisting}
for (idx = 0; idx < chunksize; idx++)
    for (blk = 0; blk < n_ranks; blk++) {
...
Figure~\ref{fig:task3_timing} shows a more balanced workload
...
A residual gradient remains because the cyclic partitioning is not perfectly
...
Dynamic (Task~2)     &  4.76 & $7.49\times$  & $1.04\times$  & 83.3\%* \\
Cyclic (Task~3)      &  4.46 & $7.99\times$  & $1.47\times$  & 79.9\% \\
\bottomrule
\end{tabular}\\[4pt]
{\footnotesize *\,Efficiency relative to 9~computing workers.
...
structure, where static partitioning cannot guarantee balance.
